...
//
// The endpoint returns JobPost resources, so each record's JSON:API `type`
// is "job_post" / "job-post". Coerce that onto the dedicated read-only
// `public-job-post` model so store.query('public-job-post', ...) resolves to
// these records — and never bleeds into the authenticated `job-post` identity
// map (whose adapter injects auth + short-circuits when logged out).
//
