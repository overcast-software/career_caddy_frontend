...
module('Unit | Route | answers/index', function (hooks) {
  setupTest(hooks);

  test('it exists', function (assert) {
    let route = this.owner.lookup('route:answers/index');
    assert.ok(route);
  });
  ...
  module('loading action', function () {
    test('self-transition returns false to suppress the loading substate', function (assert) {
      let route = this.owner.lookup('route:answers/index');
      route.routeName = 'answers.index';
      assert.false(route.loading({ from: { name: 'answers.index' } }));
    });
